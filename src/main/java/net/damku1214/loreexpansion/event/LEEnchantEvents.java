...
public class LEEnchantEvents {
    ...
    public static void procMeleeEnchants(LivingIncomingDamageEvent event, ItemStack stack) {
        ...
        multiplicativeDmgBonus *= getAmbushDmgBonus(event, stack);
        multiplicativeDmgBonus *= getCommittedDmgBonus(event, stack);
        multiplicativeDmgBonus *= getCriticalHitDmgBonus(event, stack);
        multiplicativeDmgBonus *= getEnigmaResonatorDmgBonus(event, stack);
    ...
    public static float getAmbushDmgBonus(LivingIncomingDamageEvent event, ItemStack stack) {
        float dmgBonus = 1.0f;
    ...
    public static void applyAnimaConduit(Player player) {
        ItemStack stack = player.getMainHandItem();
        int enchantLevel = stack.getEnchantmentLevel(player.level().holderOrThrow(LEEnchants.ANIMA_CONDUIT));
        player.heal(0.02f * enchantLevel * player.getMaxHealth());
    }
    ...
    public static float getCommittedDmgBonus(LivingIncomingDamageEvent event, ItemStack stack) {
        float dmgBonus = 1.0f;
    ...
    public static float getCriticalHitDmgBonus(LivingIncomingDamageEvent event, ItemStack stack) {
        float dmgBonus = 1.0f;
        ...
        if (event.getAmount() > 0 && enchantLevel > 0 && Math.random() < 0.025 * enchantLevel + 0.075) {
            dmgBonus = 3.0f;
            emitCritParticles(level, attacker, target);
    ...
    public static float getEnigmaResonatorDmgBonus(LivingIncomingDamageEvent event, ItemStack stack) {
        float dmgBonus = 1.0f;

        LivingEntity attacker = (LivingEntity) event.getSource().getEntity();
        assert attacker != null;
        if (!(attacker instanceof Player player)) return dmgBonus;
        LivingEntity target = event.getEntity();

        ServerLevel level = ((ServerLevel) attacker.level());
        int enchantLevel = stack.getEnchantmentLevel(attacker.level().holderOrThrow(LEEnchants.ENIGMA_RESONATOR));
        float soulPercentage = (float) (player.getData(LEAttachments.SOUL_DATA.get()).getSouls() / player.getAttributeValue(LEAttributes.MAX_SOULS));

        if (event.getAmount() > 0 && enchantLevel > 0 && Math.random() < (0.025 * enchantLevel + 0.125) * soulPercentage) {
            dmgBonus = 3.0f;
            emitCritParticles(level, attacker, target);
            level.playSound(null, target.blockPosition(), LESounds.CRIT.get(), SoundSource.PLAYERS);
        }
    ...
    public static void applyLeeching(LivingDeathEvent event, ItemStack stack) {
        ...
        if (enchantLevel > 0) {
            attacker.heal(target.getMaxHealth() * (0.04f * enchantLevel + 0.01f));
    ...
    public static void attemptSoulSiphon(LivingIncomingDamageEvent event, ItemStack stack) {
        LivingEntity attacker = (LivingEntity) event.getSource().getEntity();
        assert attacker != null;
        ...
        ServerLevel level = ((ServerLevel) attacker.level());
        int enchantLevel = stack.getEnchantmentLevel(attacker.level().holderOrThrow(LEEnchants.SOUL_SIPHON));
